...
pub fn sanitize_filename_with_options<S: AsRef<str>>(name: S, options: Options) -> String {
    let Options {
        windows,
        truncate,
        replacement,
...
mod tests {
    ...
    static NAMES_IS_SANITIZED: &'static [bool] = &[
        ...
        false, false, true, false, true, true, true, true, false, false, false, true, false, false,
    ...
    fn it_works() {
        ...
        for (idx, name) in NAMES.iter().enumerate() {
            assert_eq!(
                super::sanitize_filename_with_options(name, options.clone()),
                NAMES_CLEANED[idx]
            );
        }
